...
const GENRES_FILTER = ['Dramma','Commedia','Crime','Sci-Fi','Animazione','Azione','Mistero','Thriller','Fantasy','Horror']
...
export default function Library() {
  ...
  const load = async () => {
    ...
    if (genreF) q = q.ilike('genres', `%${genreF}%`)
  ...
  return (
    <div className="page">
      ...
      <div style={{ display:'flex', gap:5, marginBottom:14, flexWrap:'wrap' }}>
        ...
        {GENRES_FILTER.map(g => <button key={g} className={`chip${genreF===g?' active':''}`} onClick={()=>setGenreF(genreF===g?null:g)}>{g}</button>)}
      </div>
