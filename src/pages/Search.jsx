...
export default function Search() {
  ...
  const doSearch = useCallback(async (q) => {
    ...
    try {
      ...
      setResults(filtered); setEmpty(filtered.length === 0)
    } catch { setResults([]) }
    setLoading(false)
  }, [])
  ...
  return (
    <div className="page">
      ...
      <div className="search-bar">
        ...
        <input className="input" type="search" placeholder="Serie, anime, cartoni..." value={query} onChange={onChange} autoFocus />
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {results.map(show => {
          ...
          return (
            <button key={show.id} className="show-card" onClick={() => navigate(`/show/${show.id}`)}>
              <div className="show-card-poster">
                {poster ? <img src={poster} alt={show.name} loading="lazy" /> : <span>No img</span>}
              </div>
