...
import { getShowDetails, getSeason, getEpCredits, imgUrl, airYear } from '../lib/tmdb'
...
function CharPicker({ value, onChange, showCast, epCast }) {
  const all = [...(epCast||[]), ...(showCast||[])]
  const seen = new Set()
  const chars = all.filter(c => { if(seen.has(c.name)) return false; seen.add(c.name); return true }).slice(0,20)
  return (
    <div>
      {chars.length > 0 && (
        <div style={{ display:'flex', flexWrap:'wrap', gap:5, marginBottom:8 }}>
          {chars.map(c => (
            <button key={c.name} onClick={() => onChange(value===c.name?null:c.name)}
              style={{ display:'flex', alignItems:'center', gap:5,
                background:value===c.name?'var(--accent-dim)':'var(--surface0)',
                border:`1px solid ${value===c.name?'var(--accent)':'var(--surface1)'}`,
                color:value===c.name?'var(--accent)':'var(--dim)',
                padding:'3px 8px', fontSize:12, cursor:'pointer' }}>
              {c.profile_path && <img src={imgUrl(c.profile_path,'w45')} alt="" style={{ width:18,height:18,objectFit:'cover' }} />}
              {c.character || c.roles?.[0]?.character || c.name}
            </button>
          ))}
        </div>
      )}
      <div style={{ display:'flex', gap:6 }}>
        <input className="input" placeholder="Oppure scrivi un nome..." value={value||''} style={{ flex:1 }}
          onChange={e=>onChange(e.target.value||null)} />
        {value && <button className="btn btn-ghost btn-sm" onClick={()=>onChange(null)}>✕</button>}
      </div>
    </div>
  )
}

function EpSheet({ ep, showId, detail, isWatched, showCast, onSave, onClose }) {
  ...
  const [character,setChar]     = useState(detail?.fav_character||null)
  ...
  const [epCast,   setEpCast]   = useState([])
  const [saving,   setSaving]   = useState(false)

  useEffect(() => {
    getEpCredits(showId, ep.season_number, ep.episode_number)
      .then(d => setEpCast([...(d.cast||[]), ...(d.guest_stars||[])]))
      .catch(()=>{})
  ...
  return (
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="sheet">
        ...
        {ep.vote_average>0 && <div style={{ fontSize:12,color:'var(--muted)',marginBottom:10 }}>TMDB ⭐ {ep.vote_average?.toFixed(1)}/10{ep.runtime&&' · '+ep.runtime+'min'}</div>}
        {ep.overview && <p style={{ fontSize:12,color:'var(--dim)',marginBottom:12,lineHeight:1.5 }}>{ep.overview}</p>}

        <div className="sheet-row">
          <label style={{ display:'flex',alignItems:'center',gap:10,cursor:'pointer' }}>
            <div onClick={()=>setWatched(w=>!w)} style={{ width:42,height:22,background:watched?'var(--accent)':'var(--surface1)',position:'relative',transition:'background .2s',cursor:'pointer',flexShrink:0 }}>
        ...
        {watched && <>
          ...
          <div className="sheet-row">
            <div className="label">Emozioni (multipla)</div>
            <div className="emotions-wrap">
              {EMOTIONS.map(({e,l})=>(
                <button key={e} className={`emo-btn${emotions.includes(e)?' on':''}`} onClick={()=>toggleEmo(e)}>{e}<span>{l}</span></button>
              ))}
            </div>
          </div>
          <div className="sheet-row">
            <div className="label">Personaggio preferito</div>
            <CharPicker value={character} onChange={setChar} showCast={showCast} epCast={epCast} />
          </div>
          <div className="sheet-row">
      ...
      {open && (
        <div className="ep-list">
          {loadE
            ...
            : <>
                <div style={{ padding:'8px 14px',borderTop:'1px solid var(--surface0)',background:'var(--mantle)',display:'flex',gap:8,alignItems:'center',flexWrap:'wrap' }}>
                  <span style={{ fontSize:11,color:'var(--muted)',flexShrink:0 }}>Stagione:</span>
                  <input type="date" className="input" style={{ flex:1,minWidth:110,padding:'4px 8px',fontSize:12 }} value={st?.start_date||''} onChange={e=>onSeasonTrack(season.season_number,'start_date',e.target.value)} />
                  <input type="date" className="input" style={{ flex:1,minWidth:110,padding:'4px 8px',fontSize:12 }} value={st?.end_date||''} onChange={e=>onSeasonTrack(season.season_number,'end_date',e.target.value)} />
                  ...
                  return (
                    <div key={ep.id} className={`ep-row${done?' watched':''}`}
                      ...
                      <span className="ep-badges">
                        {emos.slice(0,2).map(e=><span key={e} style={{ fontSize:13 }}>{e}</span>)}
                        {detail?.fav_character && <span style={{ fontSize:10,color:'var(--mauve)' }}>★</span>}
                        {detail?.rating && <span style={{ fontSize:10,color:'var(--gold)' }}>{'★'.repeat(detail.rating)}</span>}
                ...
                <div className="season-actions">
                  <button className="btn btn-ghost btn-sm" style={{ flex:1 }} onClick={()=>realEps.forEach(e=>onToggle(season.season_number,e.episode_number,true))}>✓ Segna tutti</button>
                  <button className="btn btn-ghost btn-sm" style={{ flex:1 }} onClick={()=>realEps.forEach(e=>onToggle(season.season_number,e.episode_number,false))}>✗ Deseleziona</button>
                </div>
              </>
          }
        </div>
...
export default function ShowDetail() {
  ...
  const load = async () => {
    ...
    setShowCast(cast.slice(0,30))
    if (us) { setStatus(us.status); setMtype(us.media_type); setRating(us.rating); setNote(us.note||''); setFavChar(us.fav_character||null); setPlat(us.main_platform||null); setDevice(us.main_device||null); setWcount(us.watch_count||1) }
  ...
  // Auto-aggiorna stato show: plan_to_watch → watching al primo episodio marcato
  const autoUpdateStatus = useCallback(async (currentUserShow) => {
    if (!currentUserShow || currentUserShow.status !== 'plan_to_watch') return
    await supabase.from('user_shows')
      .update({ status:'watching', updated_at:new Date().toISOString() })
      .eq('user_id',user.id).eq('tmdb_id',tmdbId)
    setUserShow(p => p ? { ...p, status:'watching' } : p)
    setStatus('watching')
  }, [tmdbId, user.id])
  ...
  const handleToggle = useCallback(async (sn, en, watched) => {
    ...
    if (watched) {
      await supabase.from('user_episodes').upsert(
        { user_id:user.id, tmdb_show_id:tmdbId, season_number:sn, episode_number:en, watched_at:new Date().toISOString() },
        { onConflict:'user_id,tmdb_show_id,season_number,episode_number' }
      )
      // Auto-salva data visione
      const existing = detailsMap[key]
      if (!existing?.watched_date) {
        await supabase.from('episode_details').upsert(
          { user_id:user.id, tmdb_show_id:tmdbId, season_number:sn, episode_number:en, watched_date:today, watch_count:existing?.watch_count||1, updated_at:new Date().toISOString() },
          { onConflict:'user_id,tmdb_show_id,season_number,episode_number' }
        )
        setDetailsMap(p => ({ ...p, [key]:{ ...p[key], watched_date:today } }))
      }
      // Auto-aggiorna stato show usando il ref attuale di userShow
      setUserShow(current => {
        if (current?.status === 'plan_to_watch') {
          supabase.from('user_shows').update({ status:'watching', updated_at:new Date().toISOString() }).eq('user_id',user.id).eq('tmdb_id',tmdbId)
          setStatus('watching')
      ...
      await supabase.from('user_episodes').delete().eq('user_id',user.id).eq('tmdb_show_id',tmdbId).eq('season_number',sn).eq('episode_number',en)
    }
  }, [tmdbId, user.id, detailsMap])
  ...
  const handleSaveEp = useCallback(async ({ watched, season_number:sn, episode_number:en, ...det }) => {
    ...
    await supabase.from('episode_details').upsert(
      { user_id:user.id, tmdb_show_id:tmdbId, season_number:sn, episode_number:en, ...det, updated_at:new Date().toISOString() },
      { onConflict:'user_id,tmdb_show_id,season_number,episode_number' }
    )
  ...
  const toggleFav = async () => {
    if (isFav) { await supabase.from('user_favorites').delete().eq('user_id',user.id).eq('tmdb_id',tmdbId); setIsFav(false) }
    else { await supabase.from('user_favorites').upsert({ user_id:user.id, tmdb_id:tmdbId, title:show?.name||'', poster_path:show?.poster_path||null },{ onConflict:'user_id,tmdb_id' }); setIsFav(true) }
  }
  ...
  const runtimeStr = runtimes.length===0?null:runtimes.length===1?'~'+runtimes[0]+'min':'~'+Math.min(...runtimes)+'–'+Math.max(...runtimes)+'min'
  ...
  return (
    <div className="page" style={{ paddingTop:0 }}>
      ...
      <div style={{ marginTop:backdrop?-44:16 }}>
        <div className="detail-header">
          ...
          <div style={{ flex:1,minWidth:0 }}>
            ...
            {show.vote_average>0 && <div style={{ fontSize:12,color:'var(--muted)',marginTop:3 }}>TMDB ⭐ {show.vote_average?.toFixed(1)} ({show.vote_count?.toLocaleString()} voti)</div>}
            {avgScore && <div style={{ fontSize:12,color:'var(--gold)',marginTop:2 }}>Media ep. ★ {avgScore}/10</div>}
        ...
        <div style={{ display:'flex',gap:6,marginBottom:14 }}>
          <button className="btn btn-primary" style={{ flex:1 }} onClick={()=>setSheet(true)}>{userShow?'✎ Modifica':'+ Aggiungi'}</button>
          <button onClick={toggleFav} style={{ background:isFav?'var(--gold-dim)':'var(--surface0)',border:'1px solid '+(isFav?'var(--gold)':'var(--surface1)'),color:isFav?'var(--gold)':'var(--muted)',padding:'9px 14px',fontSize:18,cursor:'pointer' }}>★</button>
        </div>

        {show.overview && <><p style={{ fontSize:13,color:'var(--dim)',lineHeight:1.6,marginBottom:14 }}>{show.overview}</p><div className="divider" /></>}
        ...
        {userShow && totalEps>0 && (
          <div style={{ marginBottom:16 }}>
            ...
            <div className="progress-bar" style={{ height:4 }}><div className="progress-fill" style={{ width:watchedSet.size/totalEps*100+'%' }} /></div>
          </div>
        )}
        ...
        {seasons.length>0 && (
          <>
            ...
            {seasons.map(s=>(
              <SeasonBlock key={s.id} showId={tmdbId} season={s} watchedSet={watchedSet} detailsMap={detailsMap}
        ...
        {showCast.length>0 && (
          <div style={{ marginTop:16 }}>
            <div className="label" style={{ marginBottom:8 }}>Cast</div>
            <div style={{ display:'flex',gap:8,overflowX:'auto',paddingBottom:4 }}>
              {showCast.slice(0,12).map(c=>(
                <div key={c.id} style={{ flexShrink:0,width:60,textAlign:'center' }}>
                  <div style={{ width:60,height:60,background:'var(--surface0)',border:'1px solid var(--surface1)',overflow:'hidden',marginBottom:3 }}>
                  ...
                  <div style={{ fontSize:9,color:'var(--muted)',lineHeight:1.2 }}>{(c.roles?.[0]?.character||c.character||'').slice(0,18)}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {sheet && (
        <div className="overlay" onClick={e=>e.target===e.currentTarget&&setSheet(false)}>
          <div className="sheet">
            ...
            <div className="sheet-row"><div className="label">Stato</div>
              <select className="select" value={status} onChange={e=>setStatus(e.target.value)}>
                {STATUSES.map(o=><option key={o.v} value={o.v}>{o.label}</option>)}
              </select>
            </div>
            <div className="sheet-row"><div className="label">Tipo</div>
              <div style={{ display:'flex',gap:6 }}>{TYPES.map(o=><button key={o.v} className={`chip${mtype===o.v?' active':''}`} onClick={()=>setMtype(o.v)}>{o.label}</button>)}</div>
            </div>
            <div className="sheet-row"><div className="label">Voto serie (1–10 ★)</div><Stars value={rating} max={10} onChange={setRating} size={20} /></div>
            <div className="sheet-row"><div className="label">Personaggio preferito della serie</div><CharPicker value={favChar} onChange={setFavChar} showCast={showCast} epCast={[]} /></div>
            <div className="sheet-row"><div className="label">Piattaforma principale</div>
              <div className="plat-wrap">{PLATFORMS.map(p=><button key={p} className={`plat-chip${platform===p?' on':''}`} onClick={()=>setPlat(platform===p?null:p)}>{p}</button>)}</div>
            </div>
            <div className="sheet-row"><div className="label">Dispositivo principale</div>
              <div className="plat-wrap">{DEVICES.map(d=><button key={d} className={`plat-chip${device===d?' on':''}`} onClick={()=>setDevice(device===d?null:d)}>{d}</button>)}</div>
            </div>
            <div className="sheet-row"><div className="label">Volte vista</div>
              <div style={{ display:'flex',alignItems:'center',gap:12 }}>
                <button className="btn btn-ghost btn-sm" onClick={()=>setWcount(c=>Math.max(1,c-1))}>−</button>
            ...
            <div className="sheet-row"><div className="label">Note</div><textarea className="input textarea" value={note} onChange={e=>setNote(e.target.value)} /></div>
            <div className="sheet-actions">
      ...
      {epSheet && (
        <EpSheet ep={epSheet.ep} showId={tmdbId} detail={epSheet.detail}
          isWatched={watchedSet.has(`${epSheet.ep.season_number}-${epSheet.ep.episode_number}`)}
          showCast={showCast} onSave={handleSaveEp} onClose={()=>setEpSheet(null)} />
      )}
    </div>
  )
}
