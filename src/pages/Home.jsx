...
export default function Home() {
  ...
  const [epCounts,  setEpCounts]  = useState({}) // tmdb_id -> count
  ...
  const loadWatching = async () => {
    ...
    if (shows?.length) {
      setWatching(shows)
      // Fetch watched episode counts for each show
      const ids = shows.map(s => s.tmdb_id)
      ...
      if (eps) {
        const counts = {}
        eps.forEach(e => {
          counts[e.tmdb_show_id] = (counts[e.tmdb_show_id] || 0) + 1
        })
        setEpCounts(counts)
      }
    }
  ...
  return (
    <div className="page">
      <div className="home-header">
        <h1 className="home-title">TV<span className="accent">TRACK</span></h1>
        <button
          onClick={signOut}
      ...
      <div className="section-header">
        ...
        <button
          ...
          style={{ background: 'none', color: 'var(--accent)', fontSize: 13 }}
        >
          Cerca →
        </button>
      </div>
      ...
      {loadingT ? (
        ...
        <div className="trending-grid">
          {trending.map(show => (
            ...
            >
              {show.poster_path && (
                <img src={posterUrl(show.poster_path, 'w342')} alt={show.name} loading="lazy" />
              )}
